...
import { formatSajuForAI, formatPeriodForAI } from '@/lib/format-saju-ai'
...
type InterpretScope = 'general' | 'daeun' | 'seun' | 'wolun'
...
const PERIOD_SYSTEM_PROMPT = `당신은 20년 이상 경험의 한국 명리학 전문가입니다.
주어진 사주 원국과 지정된 기간 데이터만 근거로 현실적인 해석을 제공합니다.
- 지정된 기간(대운/세운/월운) 바깥으로 범위를 넓히지 마세요.
- 확정적 예언, 공포 조장, 과장 표현은 금지합니다.
- 사건 단정 대신 경향, 기회, 주의점, 실행 조언으로 답변하세요.
...
export async function POST(req: NextRequest) {
  ...
  const selectedCategory: Category = validCategories.includes(category as Category)
    ...
    : '종합'
  const validScopes: InterpretScope[] = ['general', 'daeun', 'seun', 'wolun']
  const selectedScope: InterpretScope = validScopes.includes(scope as InterpretScope)
    ? (scope as InterpretScope)
    : 'general'
  ...
  let systemContent: string = followUp ? FOLLOWUP_SYSTEM_PROMPT : SYSTEM_PROMPT

  if (selectedScope !== 'general') {
    if (!yongsin) {
      return NextResponse.json(
        { error: '기간 해석에는 yongsin 데이터가 필요합니다.' },
        { status: 400 }
      )
    }
    ...
    if (selectedScope === 'daeun') {
      if (!daeun) {
        return NextResponse.json(
          { error: '대운 해석에는 daeun 데이터가 필요합니다.' },
          { status: 400 }
        )
      }
      if (!Number.isInteger(periodIndex) || (periodIndex as number) < 0 || (periodIndex as number) > 9) {
        return NextResponse.json(
          { error: 'periodIndex는 0~9 사이의 정수여야 합니다.' },
          { status: 400 }
        )
      }

      const daeunResult = daeun as DaeunResult
      const selectedPeriod = daeunResult.periods[periodIndex as number]
      if (!selectedPeriod) {
        return NextResponse.json(
          { error: '유효하지 않은 periodIndex입니다.' },
          { status: 400 }
        )
      }

      userMessage = formatPeriodForAI({
        scope: 'daeun',
        base: periodBase,
        daeun: daeunResult,
      ...
      systemContent = PERIOD_SYSTEM_PROMPT
    } else if (selectedScope === 'seun') {
      if (!Number.isInteger(targetYear)) {
        return NextResponse.json(
          { error: '세운 해석에는 targetYear가 필요합니다.' },
          { status: 400 }
        )
      }

      const seunFortune = calculateYearlyFortune(saju as FullSajuResult['saju'], targetYear as number)
      userMessage = formatPeriodForAI({
        scope: 'seun',
        base: periodBase,
      ...
      if (!Number.isInteger(targetYear) || !Number.isInteger(targetMonth)) {
        return NextResponse.json(
          { error: '월운 해석에는 targetYear와 targetMonth가 필요합니다.' },
          { status: 400 }
        )
      }
      if ((targetMonth as number) < 1 || (targetMonth as number) > 12) {
        return NextResponse.json(
          { error: 'targetMonth는 1~12 사이여야 합니다.' },
          { status: 400 }
        )
      }

      const wolunFortune = calculateMonthlyFortune(
        saju as FullSajuResult['saju'],
      ...
      userMessage = formatPeriodForAI({
        ...
        base: periodBase,
        targetYear: targetYear as number,
        targetMonth: targetMonth as number,
        fortune: wolunFortune,
      })
      systemContent = PERIOD_SYSTEM_PROMPT
    }
  } else {
  ...
  const scopedMaxTokens: Record<Exclude<InterpretScope, 'general'>, number> = {
    daeun: 800,
    seun: 650,
    wolun: 500,
  }
  const modelParams = {
    ...
    max_tokens: selectedScope !== 'general'
      ? scopedMaxTokens[selectedScope]
      : (followUp ? 600 : (isComprehensive ? 2000 : 1200)),
